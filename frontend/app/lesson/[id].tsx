    ...
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]}>
      ...
      >
        ...
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          {/* Media */}
          {current?.media_base64 && current.type !== "word_order" && (
            <Image
                ...
                return (
                  ...
                  >
                    <Text
                      style={[
                        styles.optText,
                        (selected || isCorrectOpt || isWrongPick) && { color: "#fff" },
                      ]}
                    >
                      {opt}
                    </Text>
              ...
              <View style={styles.wordBank}>
                {pickedWords.map((w, i) => (
                  <TouchableOpacity
                    key={`picked-${i}`}
                    disabled={!!result}
                    style={[styles.wordChip, styles.wordChipPicked]}
                    onPress={() => {
                      const removed = pickedWords[i];
                      const np = [...pickedWords];
                      np.splice(i, 1);
                      setPickedWords(np);
                      setAvailableWords((a) => [...a, { word: removed, key: Date.now() + i }]);
                      setAnswer(np);
                    }}
                  >
                ...
                {pickedWords.length === 0 && (
                  <Text style={styles.bankHint}>Tocca le parole per formare la frase</Text>
                )}
              </View>
        ...
        <View style={styles.footer}>
          {!result ? (
            <TouchableOpacity
              ...
              style={[styles.primaryBtn, (answer === null || answer === undefined) && { opacity: 0.5 }]}
              onPress={submit}
...
function AudioVideoBox({ mime, base64, url, type }: { mime?: string; base64?: string; url?: string; type: string }) {
  const source = base64 ? `data:${mime || (type === "audio" ? "audio/mp3" : "video/mp4")};base64,${base64}` : url;
  // Use expo-av lazily to avoid web issues
  ...
  if (!source) return <Text style={{ color: COLORS.textMuted }}>Nessun media disponibile.</Text>;
  ...
  if (type === "audio") {
    ...
    return (
      <TouchableOpacity style={styles.audioBox} onPress={play} testID="audio-play">
        <Ionicons name={playing ? "pause-circle" : "play-circle"} size={64} color={COLORS.primary} />
...
const styles = StyleSheet.create({
  ...
  bankHint: { color: COLORS.textDisabled, fontStyle: "italic" },
  wordPool: { marginTop: 12, flexDirection: "row", flexWrap: "wrap", gap: 8 },
  wordChip: {
    backgroundColor: COLORS.surface,
    borderWidth: 2,
    borderBottomWidth: 4,
  ...
  audioHint: { marginTop: 6, color: COLORS.textMuted, fontWeight: "600" },
